...
class LSHADE_Q:
    ...
    def run(self):
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            # 选择动作，获取 F 和 CR
            state = torch.tensor(self.population[0], dtype=torch.float32).to(self.get_device())
            ...
            for i in range(self.pop_size):
                ...
                if selection:
                    epoch_new_fit_sum += np.sum(new_fit)
                else:
                    epoch_no_improve_pop += 1
                    epoch_reward -= 1  # 如果没有选择成功，给予负奖励

            # 计算奖励在所有个体选择成功后的总和

            epoch_reward += -(epoch_new_fit_sum - np.sum(previous_fit))  # 计算适应度变化的奖励
            # 更新帕累托前沿
            self.update_pareto_front()
    ...
    def learn(self):
        ...
        rewards = torch.tensor(rewards, dtype=torch.float32).unsqueeze(1).to(self.get_device())  # 奖励，增加一个维度以匹配目标Q值的形状
        ...
        with torch.no_grad():  # 在计算目标Q值时不需要计算梯度
            target_Q = self.dqn(next_states)  # 计算下一个状态的Q值
            max_target_Q, _ = torch.max(target_Q, dim=1, keepdim=True)  # 获取下一个状态的最大Q值
            target = rewards + self.gamma * max_target_Q * (1 - dones)  # 计算目标Q值，考虑折扣因子和终止状态

        # 计算损失
        loss = self.criterion(current_Q, target)  # 使用损失函数计算当前Q值与目标Q值之间的差异
